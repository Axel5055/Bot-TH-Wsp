...
// 🔧 Función para manejar respuestas con banderas
async function manejarRespuesta(msg) {
    try {
        if (!msg.hasQuotedMsg) return; // Solo si es respuesta

        const emoji = msg.body.trim();
        const idioma = FLAG_LANG[emoji];
        if (!idioma) return; // Ignorar si no es bandera

        const quoted = await msg.getQuotedMessage();
        if (!quoted?.body) {
            consola.warn("⚠️ El mensaje citado está vacío o no se pudo obtener.");
            return;
        }

        consola.info(`✏️ Traduciendo mensaje al idioma ${idioma}:`, quoted.body);

        const traduccion = await traducir(quoted.body, idioma);
        if (!traduccion) {
            consola.error("❌ Traducción falló.");
            return;
        }

        // Enviar traducción como respuesta al mensaje original
        ...
        consola.error("🔥 Error en manejarRespuesta:", error);
    }
}
...
function traductor() {
    ...
    consola.info("🔄 Traductor de respuestas inicializado.");

    // Escuchar mensajes (respuestas con banderas)
    sony.on("message", manejarRespuesta);
